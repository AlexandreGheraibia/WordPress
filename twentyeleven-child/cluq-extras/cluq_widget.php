...
<?php
	...
	*/
	...
	public function widget( $args, $instance ) {
		...
		$menuContent='';
      	$menuContent.='<ul>';
		$mContent='<select class="uq_drop_list" name="uq" >';
		$mContent.='<option value="">-- choisissez une union de quartiers --</option>';
		$i=0;
        foreach($this->getWidgetFields() as $field){
			
				if(isset($instance[$field['id']])&& $instance[$field['id']]=='on'){
					
				$menuContent.='<li class="uq_menu_accordion">'.$field['label'];
				$menuContent.='<ul class="uq_menu_panel">';
				$mContent.='<option value="'.$i.'">'.$field['label'];
		...
		$mContent.='</select>';
		$menuContent.='</ul>';
		$menuContent=$mContent.$menuContent;
		echo $menuContent;		
